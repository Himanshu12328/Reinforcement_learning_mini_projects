...
class Agent_DQN(Agent):
    ...
    """
        ...
        with torch.no_grad():
            ...
            # print(observation)
            # print(self.q_network(tensor(observation).float()))
            # q1, r1 = self.q_network(tensor(observation).float())
            # print(q1)
            # print(r1)
            # print(self.q_network(tensor(observation).float()).data.cpu().shape)
            q, argq = self.q_network(tensor(observation).float()).data.cpu().max(1)
            # q, argq = ((self.q_network(tensor(observation).float()).data.cpu()).numpy()).max(1)
            # Increase the probability for the selected best action
            self.probability_list[argq[0].item()] += 1 - self.epsilon
            # self.probability_list[argq.item()] += 1 - self.epsilon
    ...
    def optimize_network(self):
        ...
        if len(self.replay_memory) < self.batch_size:
            print("Pratyush {}".format(self.batch_size))
            return 0
